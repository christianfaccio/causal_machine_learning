...
"""
Modified CEVAE implementation that captures decoder distribution parameters during training.
...
class Model(PyroModule):
    ...
    def get_decoder_params(self, z, t):
        """
        Extract the parameters (means and variances) from the decoder networks.
        ...
        """
        with torch.no_grad():
            # Get x distribution parameters (decoder from z to x)
            x_loc, x_scale = self.x_nn(z)
            
            # Get y distribution parameters (decoder from (z,t) to y)
            y0_params = self.y0_nn(z)
            y1_params = self.y1_nn(z)
            
            params_dict = {
                'x_mean': x_loc.mean(dim=0),  # Average over batch
                'x_var': (x_scale ** 2).mean(dim=0),  # Average over batch
                'y0_params': [p.mean(dim=0) for p in y0_params],  # Parameters for t=0
                'y1_params': [p.mean(dim=0) for p in y1_params],  # Parameters for t=1
            }
            
            # For outcome distributions that have mean/variance structure
            if len(y0_params) >= 2:  # Normal, Laplace, StudentT
                params_dict['y0_mean'] = y0_params[0].mean(dim=0) if len(y0_params) > 1 else y0_params[0].mean(dim=0)
                params_dict['y0_var'] = (y0_params[1] ** 2).mean(dim=0) if len(y0_params) > 1 else torch.tensor(0.0)
                params_dict['y1_mean'] = y1_params[0].mean(dim=0) if len(y1_params) > 1 else y1_params[0].mean(dim=0)
                params_dict['y1_var'] = (y1_params[1] ** 2).mean(dim=0) if len(y1_params) > 1 else torch.tensor(0.0)
            elif len(y0_params) == 1:  # Bernoulli, Exponential
                params_dict['y0_mean'] = y0_params[0].mean(dim=0)
                params_dict['y1_mean'] = y1_params[0].mean(dim=0)
...
class CEVAE(nn.Module):
    ...
    Example::
        ...
        training_info = cevae.fit(x_train, t_train, y_train, track_params=True)
        ite = cevae.ite(x_test)  # individual treatment effect
        ate = ite.mean()         # average treatment effect
        
        # Access decoder parameters from training
        decoder_params_history = training_info['decoder_params']
    ...
    def fit(
        ...
        :param bool track_params: Whether to track decoder parameters during training.
            Defaults to False.
        :param int track_every: Track parameters every this-many steps when track_params=True.
            Defaults to 10.
        :return: Dictionary containing 'losses' and optionally 'decoder_params' if track_params=True
        """
        ...
        losses = []
        decoder_params_history = [] if track_params else None
        
        step_count = 0
        for epoch in range(num_epochs):
            for x_batch, t_batch, y_batch in dataloader:
                x_batch = self.whiten(x_batch)
                loss = svi.step(x_batch, t_batch, y_batch, size=len(dataset)) / len(dataset)
                ...
                # Track decoder parameters if requested
                if track_params and step_count % track_every == 0:
                    self._track_decoder_params(x_batch, t_batch, decoder_params_history, step_count)
                
                step_count += 1
                
        result = {'losses': losses}
        if track_params:
            result['decoder_params'] = decoder_params_history
            
        return result

    def _track_decoder_params(self, x_batch, t_batch, params_history, step):
        """
        Track decoder parameters for the current batch.
        """
        self.model.eval()
        with torch.no_grad():
            # Sample latent variables from the model
            with pyro.plate("data", x_batch.size(0)):
                z = pyro.sample("z", self.model.z_dist())
            
            # Get decoder parameters
            params = self.model.get_decoder_params(z, t_batch)
            params['step'] = step
